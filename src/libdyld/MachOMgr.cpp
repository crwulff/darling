...
void MachOMgr::registerClassHook(ClassRegisterHookFunc* func)
{
	Darling::RWMutexWriteLock l(m_lock);

	m_classHooks.insert(func);
}

void MachOMgr::registerNativeClass(void *cls)
{
	for (ClassRegisterHookFunc* func : m_classHooks)
	{
		func(cls);
	}
}
...
void MachOMgr::notifyAdd(MachOObject* obj)
{
	...
	if (it != m_objectsInOrder.end())
		index = std::distance(m_objectsInOrder.begin(), it);
