...
#ifdef DEBUG
		elf.addSection(seg->segname, (void*)(seg->vmaddr + m_slide), seg->vmsize, 0);
#endif
	}
}
...
void MachOObject::readSymbols()
{
	for (const MachO::Symbol& sym : m_file->symbols())
	{
		...
		if (sym.name[0] == '_' && sym.name[1] == '_')
		{
			...
			elf.addSymbol(&sym.name[1], addr);
		}
		else
		{
			elf.addSymbol(sym.name, addr);
		}
#endif
	}
}
...
#ifdef DEBUG
		elf.addSymbol(e.name, (void*)e.addr);
#endif
	}
}
...
void* MachOObject::performBind(MachO::Bind* bind)
{
	if (bind->type == BIND_TYPE_POINTER || bind->type == BIND_TYPE_STUB)
	{
		...
		if (!addr && !bind->is_weak)
		{
			...
			{
				int status;
				char* demangled_name = abi::__cxa_demangle(bind->name.c_str(), 0, 0, &status);

				ERROR().write("Creating a fake implementation for " + bind->name.substr(1) +
						((demangled_name) ? (std::string(" (") + demangled_name + ")" ) : ""));
